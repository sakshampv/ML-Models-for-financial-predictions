...
df_train = pd.read_excel (r'./ML_Data/TrainingData2.xlsx', sheetname = "1_MONTH_LAG")
y = df_train['PX_LAST']
from sklearn.preprocessing import StandardScaler
sc = StandardScaler()
qq = pd.read_csv('1monthmodifieddata.csv')
...
def backwardElimination(x, sl):
    ...
    #with open('Stockwise_reg_12_mo/Summary_' + str(sl) + "_" +colnams[k]+'.csv', 'w') as fh:                
...
for SL in slarray:
 ...
 for stock in c:
    ...
    col3 = list(X_Modeled.columns)
    coefs = []
    for i in range(len(colnams2)):
            flg = 0
            for j in range(len(col3)):
                if( colnams2[i] == col3[j]):
                    coefs += [regressor.coef_[j]]
                    flg = 1
 ...
 for i in range(len(colnams2)): 
   final_results[colnams2[i]] = coff[:,i]


 final_results = final_results.drop(columns = ['const'], inplace = False) 
 final_results.to_csv("1moStockwise_Regression_Results_" +str(SL) +  "_.csv")
